...
updated: 2026-06-09
type: concept
tags: [architecture, cli, terminal, ux, tui, ink, setup, prompt-size, partial-compress, undo-rewind, desktop-cli, fuzzy-picker, unified-sessions-overlay, native-windows-confirm-modal, tui-subprocess-stdin-devnull, terminal-config-env-map, fallback-chain-canonical, portal-url-persist, dashboard-public-url-redirect-uri, max-concurrent-sessions-cap, hermes-ink-mouse-tracking-teardown]
sources: [cli.py, hermes_cli/, ui-tui/, tui_gateway/, hermes_cli/partial_compress.py, hermes_cli/prompt_size.py, hermes_cli/mcp_startup.py, hermes_cli/active_sessions.py, hermes_cli/config.py, hermes_cli/dashboard_register.py, apps/desktop/, hermes_state.py, ui-tui/packages/hermes-ink/src/ink/components/App.tsx, ui-tui/src/lib/fuzzy.ts, web/src/lib/fuzzy.ts, ui-tui/src/components/activeSessionSwitcher.tsx, scripts/check_subprocess_stdin.py]
---

> **2026-06-09 增量（hermes-agent `a5d05cf30`）— Native Windows confirm modal + TUI subprocess stdin EOF 全面修补 + terminal config env bridge + max session cap + hermes-ink SGR teardown**：
>
> - **Native Windows confirm modal #33961**（`ab98818e5 + d66bac5a1 + 714183530 + b5f8996cc`）—— destructive-slash confirm modal 之前在 `sys.platform == "win32"` 短路 fallback 到裸 `input()`。daemon thread 内 `input()` 与 prompt_toolkit 主线程 stdin 拥有权死锁，`/reset` 卡 + Ctrl-C 被吞。修：native Windows 现走 modal 同 Linux/macOS via `self._app.loop.call_soon_threadsafe`，平台短路移除，新 docstring "Platform note (Windows — issue #33961)" at `cli.py:6221-6240`。新 nested `_stdin_fallback()` closure at `:6254-6262`：`win32 and not in_main_thread` 时返 None（clean cancel）避 raw `input()` 死锁；否则 `self._prompt_text_input("Choice [1/2/3]: ")`。`_DESTRUCTIVE_SKIP_TOKENS = frozenset({"now", "--yes", "-y"})` at `:8268` 改为 general non-interactive escape hatch（不再 Windows-specific workaround）。`tests/cli/test_slash_confirm_windows.py` 重写到新 contract（parametrized `_run_on_daemon` harness for linux/win32）+ 真 helper-chain 测试。
> - **TUI 模式 subprocess stdin EOF crash 全面修补**（`d1f23bb2d fix: prevent TUI gateway stdin EOF crash across all TUI-context subprocess calls`）—— TUI 模式下 gateway 子进程与 Node 父进程经 stdin JSON-RPC 通信，任何继承 stdin 的 `subprocess.run/Popen` 都可能 race-read EOF 致 gateway 干净 exit on mid-tool-execution（同 #14036 / PR #39257 根因）。本 commit 跨 **28 文件 85 个 call site** 加 `stdin=subprocess.DEVNULL`：agent/tools/plugins/tui_gateway 全覆，CLI/tests/scripts/gateway/cron 不动。CI guard `bddab61bc + 8bb60ff03`：新 `scripts/check_subprocess_stdin.py` regex scanner（`subprocess\.(run|Popen)\s*\(["\'a-zA-Z_\[\(]` 不命中 comment / docstring 提及，`input=` calls 跳过）+ pytest wrapper `tests/tools/test_subprocess_stdin_guard.py`。Override `2c1aaa9cb + dba6380ca`：`agent/anthropic_adapter.py:1223-1228` interactive `claude setup-token` 必须保 inherited stdin，加 inline `noqa: subprocess-stdin` marker；`EXEMPT_MARKER` 在 call 文本或上方 4 行注释命中即豁免。
> - **Terminal config → env 桥共享 helper**（`f8adefdeb fix(tui): apply terminal backend config before launch`）—— TUI/dashboard 子进程现也走 classic CLI 同款 `terminal.*` config-to-env bridge，但不付 `cli.py` 启动副作用。`hermes_cli/config.py:5156-5184 TERMINAL_CONFIG_ENV_MAP` 全 `terminal.*` key → env var twin（`backend → TERMINAL_ENV / modal_mode → TERMINAL_MODAL_MODE / cwd → TERMINAL_CWD / timeout / lifetime_seconds / docker_{image,volumes,env,mount_cwd,extra_args,run_as_host_user,persist,orphan_reaper} / singularity/modal/daytona images / ssh fields / container CPU/memory/disk/persistent / sandbox_dir / persistent_shell`）；`terminal_config_env_var_for_key(key):5193-5198` 剥 `terminal.` 前缀查表；`apply_terminal_config_to_env(env, config, override):5201-5239` 读 `read_raw_config()` 决定 config.yaml authoritative；JSON-encode list/dict via `_terminal_env_value:5187-5190`；`set_config_value at :6131-6133` 共享同 helper 删旧 `_config_to_env_sync` dict。Wired：`hermes_cli/main.py:1825-1832 _launch_tui` + `hermes_cli/web_server.py:8573-8580 _resolve_chat_argv` spawn 前调用，wrap try/except 仅 debug log 不挡 launch。
> - **TUI fallback chain canonical**（`520b59db1 + 4b073d090 fix(tui)`）—— TUI agent factory 现走 shared `get_fallback_chain` helper 与 `HermesCLI.__init__` 和 `gateway/run.py` 一致。之前 TUI `cfg.get("fallback_providers") or cfg.get("fallback_model")` 二选一发散。`_load_fallback_model() at tui_gateway/server.py:2592-2603` thin delegate `from hermes_cli.fallback_config import get_fallback_chain; return get_fallback_chain(_load_cfg())`；`_agent_fallback_model(agent):2606-2612` 优先 `agent._fallback_chain`（空 list 视为故意）→ `_fallback_model` → `_load_fallback_model()`。
> - **`hermes dashboard register` 显式 `--portal-url` 持久 + `--redirect-uri` 派生 `HERMES_DASHBOARD_PUBLIC_URL`**（`a46462ec6 fix(cli) (#42435)`）—— `hermes_cli/dashboard_register.py:271-275 custom_portal_supplied = bool(isinstance(portal_override, str) and portal_override.strip())` 与 resolved `portal_base_url` 解耦；写入决策 `:354-372`：custom 时 `should_write_portal = existing_portal != portal_base_url`（总写）；否则旧保守 `not existing_portal and portal_base_url.rstrip("/") != "https://portal.nousresearch.com"`。`--redirect-uri` 经 `urlparse` 取 ORIGIN（scheme://netloc）at `:394-403`，仅 http/https + netloc 非空；持久 ORIGIN 不含 callback path（`dashboard_auth/routes._redirect_uri` 自重建经 `/auth/callback`）。`_print_post_register_hint:184-198` 加 `public_url=""` 参数 + 打印 `"HERMES_DASHBOARD_PUBLIC_URL=" + str(public_url)`。
> - **`hermes dashboard register` 幂等**（`52ae9d9f0 feat(dashboard) (#42455)`）—— 重跑现在更新已存 record 而非新建副本。stable key：先前持久化的 `HERMES_DASHBOARD_OAUTH_CLIENT_ID`。`_register_self_hosted_client at dashboard_register.py:93` 签名加 `existing_client_id: Optional[str] = None`；body `:117-127` `name` 空时省、`client_id` 仅在 `existing_client_id` set 时含；`cmd_dashboard_register at :230` `:269-280` 读 env、`:282-290` name 选择（显式 `--name` 赢；re-run 保留；否则 auto-generate）；区分 "Updated dashboard" vs "Registered dashboard"。
> - **可选 `max_concurrent_sessions` cap**（`639c1e363 feat(sessions): add optional max session cap`）—— 跨 CLI/TUI/dashboard/gateway 单主机/单 profile runtime 守护；fail-open。**新建** `hermes_cli/active_sessions.py 321 行` lease registry at `$HERMES_HOME/runtime/active_sessions.json` + `.lock`（POSIX `fcntl.flock` / Windows `msvcrt.locking`）+ `psutil.create_time()` 抗 pid recycling。CLI 入口：`HermesCLI.__init__:3465 self._active_session_lease = None` + `_claim_active_session(surface, *, stderr):3494` + `_release_active_session():3522` atexit 注册；`HermesCLI.run() at :10540` + 单 query 路径 `:13328` 都 claim，rejection exit 1。TUI 入口 `session.start / .resume / .branch` 三处 claim，rejection JSON-RPC `4090`。Gateway 入口在 `_running_agents[_quick_key] = _AGENT_PENDING_SENTINEL` 之前 claim。
> - **hermes-ink SGR mouse-tracking leak fix #42527**（`1e5ff4a57 fix(hermes-ink): disable mouse tracking on raw-mode teardown`）—— raw-mode teardown 关了 modifyOtherKeys / kitty keyboard / focus reporting / bracketed paste，但留 DEC mouse tracking（1000/1002/1003/1006）asserted。raw mode 关后无 reader，terminal 把 hover report 当文本回显（`35;col;row M` flood over the prompt）。`ui-tui/packages/hermes-ink/src/ink/components/App.tsx:343-351` teardown 写 `DISABLE_MOUSE_TRACKING` 在 `DBP` 后、`stdin.setRawMode(false)` 前；import 加 `DISABLE_MOUSE_TRACKING` 与 `DBP/DFE/EBP/EFE/SHOW_CURSOR` 并列 at `:33`；`:312-327` re-entry 经 `setImmediate(() => { instances.get(this.props.stdout)?.reassertTerminalModes() })` 让 reassert 落在 alt-screen enable 之后；`reassertTerminalModes` gate on `altScreenActive` + idempotent；与 `handleSuspend() at :498-503`（SIGSTOP 前已写 `SHOW_CURSOR + DFE + DISABLE_MOUSE_TRACKING`）对齐。
>
> 详见 [[2026-06-09-update#12-tui-cli-cluster]]、[[2026-06-09-update#13-hermes-ink]]、[[2026-06-09-update#15-sessions-dashboard]]。
>
