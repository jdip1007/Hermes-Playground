...
updated: 2026-06-09
type: concept
tags: [gateway, architecture, module, telegram, discord, messaging, qq, teams, line, simplex, ntfy, wecom, proxy, api-server, debounce, dm-topic, stream-event-protocol, per-platform-streaming, photon-imessage, photon-grpc-native, simplex-1-1-0, image-generate-auto-deliver, max-concurrent-sessions-cap, terminal-bare-fenced-block, markdownv2-edit-finalize, file-attach-rpc]
sources: [gateway/run.py, gateway/platforms/, gateway/platforms/api_server.py, gateway/delivery.py, gateway/platforms/telegram.py, gateway/stream_events.py, gateway/stream_dispatch.py, plugins/platforms/, plugins/platforms/photon/, plugins/platforms/simplex/, hermes_cli/config.py, hermes_cli/active_sessions.py, gateway/platforms/wecom_callback.py, tui_gateway/server.py]
---
...
Gateway 是 Hermes Agent 的**统一消息网关**，截至 2026-06-09 支持 **24 个消息平台**（含 7 个插件平台：IRC / Teams / Google Chat / LINE / SimpleX Chat / ntfy / **Photon iMessage**（v0.16.0 起 gRPC-native）），从单一进程管理所有平台的连接和消息分发。

> **2026-06-09 跨日同步增量（hermes-agent `a5d05cf30`）— Photon gRPC-native iMessage 收官 + SimpleX 1.1.0 + 跨平台 UI 改良**：
>
> - **Photon iMessage 通道完全 gRPC-native**（PR #42397 / #42732 等 20+ commit；`4e4d27875 feat(photon): gRPC-native iMessage channel (no webhook)` 主轴 1317+/1170−）—— 删 `aiohttp` webhook receiver + HMAC-SHA256 验签 + 5 个 `PHOTON_WEBHOOK_*` env + CLI `webhook register/list/delete` 三子命令；改 sidecar 端 `GET /inbound -> 200 NDJSON stream + one consumer at a time + 25s heartbeat` 长连接 + Python 端 `_inbound_loop` 长连接消费 `X-Hermes-Sidecar-Token` + 空行 heartbeat + 指数退避重连。验证：`plugins/platforms/photon/{adapter.py(1162行), auth.py(1065行), cli.py(387行), sidecar/index.mjs(542行), sidecar/package.json(24行), plugin.yaml(76行 v0.1.0→0.2.0)}`。`adapter.py:158 class PhotonAdapter` docstring：*"Bidirectional bridge to Photon Spectrum via the Node spectrum-ts sidecar"*。生产硬化 `92dfd70d6 (#42732)`：5 条 npm `overrides` 钉补丁版钳 protobufjs GHSA-66ff-xgx4-vchm + 4 个 @opentelemetry OTLP CVE / `crypto.timingSafeEqual` 替 `!==` token compare / 2 MiB body size cap / re-subscribe loop + jitter / dedup 严格 LRU。入站附件/语音 base64 inline 20MiB cap → cache image/audio/document（`314af28e8 + dbf2470d4`）；出站 6 方法 override 经 spectrum-ts `attachment()/voice()` builder（`4615e08d3 (#42397)`）；spectrum-ts API 收敛 4 commit 走 `spectrumText/spectrumReply/spectrumTyping` 后删 replyTo + `flattenGroups: true`（`fdf48c63c/e79e44af7/0d25cae04/2130ef68b`）；Home channel `PHOTON_HOME_CHANNEL` 支持裸 E.164 + DM GUID + setup `_autoconfigure_access(phone)` 自动 allowlist + `tools/send_message_tool.py:41 _PHONE_PLATFORMS` 加 photon（`064665688/92179352f/3b983e779`）；setup 体验：彩色 banner / `assignedPhoneNumber` 取号 / `~/.hermes/auth.json:credential_pool.photon_user` 持久化 phone numbers（`e9b26c7c8/84e4b4b9a/b58ff9345`）；User API 全迁 Spectrum backend Basic auth（`auth.py:71 DEFAULT_SPECTRUM_HOST="https://spectrum.photon.codes"`，`033765890`）；device client_id `hermes-agent→photon-cli` 修 400 invalid_client + token 双重 validate `validate_photon_token(token)` 先 `/api/auth/get-session` 再 `/api/projects/`（`37561c214`）；outbound 健壮性 `strip_markdown` + `_send_with_retry(max_retries=2, base_delay=2.0)` 指数退避（`637cf94be`）。
> - **SimpleX 1.1.0 大演进**（PR #27978 salvage `0c2e81df0` 984+/222−）—— 群组 `SIMPLEX_GROUP_ALLOWED` 默 disabled + `chat_id=group:<id>`；入站文件 rcvFileDescrReady→rcvFileComplete 延迟流程 `_pending_file_transfers`；原生出站媒体 `send_image/video/document/voice` 全按 numeric ID `/_send … json [...]`；`MEDIA:<path>` tag 自 reply 抽取按扩展名分发；`HERMES_SIMPLEX_TEXT_BATCH_DELAY` 默 0.8s burst 批处理；`SIMPLEX_AUTO_ACCEPT` 默 true contact request；`_pending_responses + _corr_counter` 关联追踪。`plugins/platforms/simplex/{adapter.py(1308 行), plugin.yaml:4 v1.0.x→1.1.0}`。**不动 core**。
> - **跨平台 Gateway 体验改良**：`9351cbafa fix(gateway): auto-deliver image_generate output as native media (#42616)` — `gateway/run.py:691-695 _AUTO_APPEND_MEDIA_TOOL_NAMES = {"text_to_speech", "text_to_speech_tool", "image_generate"}` + `:702 _JSON_MEDIA_TOOL_PATH_FIELDS = ("host_image", "image", "agent_visible_image")` 优先级序，extraction loop `:766-787` 让 JSON artifact `{"image": "/abs/path.png"}` 也走 auto-deliver path（之前 image 交付静默依赖模型重述路径）；`3705625b7 + 8d99b5bc4 feat/fix(gateway): bare fenced code blocks (#42576/#42729)` — 终端命令在 markdown-capable 平台渲为 ``` 围栏块（**无** language tag 因 Slack mrkdwn 把 "bash" 渲为字面 first code line），gate on adapter `supports_code_blocks: bool`（telegram/slack/whatsapp/matrix/weixin/feishu/discord plugin 7 个设 True），verbose mode full + non-verbose all/new mode 单行 capped；`6d2732e78 + f4531feee fix(gateway+telegram): MarkdownV2 edit finalize` — adapter 加 `REQUIRES_EDIT_FINALIZE: bool` 让 progress edit 也跑 `format_message()`（Telegram + DingTalk 设 True），fallback 失败时 log + `_strip_mdv2()` 干净纯文本而非 raw escape sequence；`_strip_mdv2` 顺序修先剥标准 `**bold**` 再 MarkdownV2 `*bold*` 防 `**` 半剥成 `*`。
> - **Sessions Cap**（`639c1e363 feat(sessions): add optional max session cap`）—— 新顶层 config `max_concurrent_sessions` 跨 CLI/TUI/dashboard/gateway。**新建** `hermes_cli/active_sessions.py 321 行` lease registry at `$HERMES_HOME/runtime/active_sessions.json` + `.lock`（POSIX `fcntl.flock` / Windows `msvcrt.locking`）+ `psutil.create_time()` 抗 pid recycling，fail-open。Gateway claim site at `gateway/run.py:7298` 在 `_running_agents[_quick_key] = _AGENT_PENDING_SENTINEL` 之前；`_active_session_limit_message(session_key) at :3403` fast in-process check；release at `_release_running_agent_state`；shutdown at `:5808` 显式 release 全 lease；TUI 同样在 `session.start/.resume/.branch` 三入口 claim，rejection 返 JSON-RPC `4090`。
> - **Desktop ↔ Gateway file.attach RPC**（`dbbd1d4d0 feat(desktop+gateway): remote-gateway file attachments via file.attach`）—— `DESKTOP_BACKEND_CONTRACT 1→2`，新 `@method("file.attach") at tui_gateway/server.py:5786-5823` 让 non-image `@file:` ref 在 remote 模式时 byte 上传 stage 到 `<workspace>/.hermes/desktop-attachments/`；helper 8 个 `_format_ref_value / _attachment_ref_path / _desktop_attachment_dir / _sanitize_attachment_name / _unique_attachment_path / _resolve_gateway_attachment_path / _decode_attachment_data_url / _stage_session_file_attachment`。
>
> 详见 [[2026-06-09-update#1-photon-grpc-native]]、[[2026-06-09-update#5-simplex-1-1-0]]、[[2026-06-09-update#14-gateway-ui-cluster]]、[[2026-06-09-update#15-sessions-dashboard]]。
