...
核心理念：**Hooks 处理事件通知，Plugins 处理功能扩展——两者互补。** [1]

## 架构原理

### Gateway Hooks — 事件驱动

Gateway Hooks 是一个**轻量级事件系统**，在网关生命周期的关键点触发处理器 [1]：
...
Plugin System 支持插件注册**工具、钩子回调、CLI 子命令**，以及向对话注入消息 [1]。
...
## PluginContext 新增 API（v0.10.0，2026-04-16） [1]
...
### v0.12.0+ — `ctx.llm`：插件直接调用宿主模型（5aa755e） [1]
