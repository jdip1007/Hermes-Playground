...
现代 LLM 在一次响应中经常返回多个工具调用（parallel tool calling）[1]。Hermes 的设计目标是：**在确保安全的前提下最大化并行度，减少总等待时间**[1]。

传统的做法要么全部串行（慢），要么全部并行（危险）。Hermes 采用了**三层安全检测 + 路径作用域分析**的智能并行策略[1]。
...
并发安全检测算法在 `agent/tool_dispatch_helpers.py` 中实现，`_should_parallelize_tool_batch()` 定义于 line 103[1]。

### 2.1 MCP 工具的并行豁免

不在 `_PARALLEL_SAFE_TOOLS` 列表里的工具未必立即降级。`agent/tool_dispatch_helpers.py:141-143` 会调用 `_is_mcp_tool_parallel_safe()`，后者委托 `tools/mcp_tool.py` 的 `is_mcp_tool_parallel_safe()`[1]：

- 若该工具来自一个配置了 `supports_parallel_tool_calls: true` 的 MCP 服务器，则**视为可并行**[1]。
...
Hermes 采用**保守默认**策略：任何不确定都降级为串行[1]。
...
1. **零竞态条件** — 路径重叠检测防止同时写入同一文件[1]
2. **保守默认** — 未知情况降级串行，不会出错[1]
3. **用户交互保护** — `clarify` 等工具永远串行，避免混乱[1]
4. **最大线程限制** — 8 个工作线程防止资源耗尽[1]
