...
> **NEW 2026-05-27**：dashboard 从只支持 loopback `_SESSION_TOKEN` 单一鉴权扩展为可插拔 OAuth provider 闸门。Phase 0–7 同日合入 master（19 个 `feat(dashboard-auth*)` + 多个 fix/test，commits `8773bbf` → `2fc4615`）[1]。
...
`hermes_cli/dashboard_auth/__init__.py`、`hermes_cli/dashboard_auth/middleware.py:32`、判定见 `8773bbf` + `949ad95` 两个 phase-0 commit。[1]

## 模块结构[1]

`hermes_cli/dashboard_auth/`（10 文件、1868 行）：[1]
...
Nous 官方 provider：`plugins/dashboard_auth/nous/__init__.py`（582 行） + `plugin.yaml`。[1]

## Provider 协议（ABC）[1]

`base.py:65-122` 定义 5-方法生命周期：[1]
...
`cookie_payload` 中的 cookie 必须 HttpOnly + Secure（HTTPS 时）+ SameSite=Lax + TTL ≤ 10 分钟。[1]
...
检查 5 个 abstract method 已实现、2 个 attr 非空、`__abstractmethods__` 已清空。失败抛 `TypeError`，破窗插件不上线。[1]
...
- **list_providers / get_provider** 是 middleware 与 `/api/auth/providers` 端点的数据源[1]。

## Middleware 闸门[1]

`middleware.py:32-46` 公开路由白名单（前缀匹配，order matters）：[1]
...
`auth_required = False` 时整个 middleware no-op，沿用 legacy `_SESSION_TOKEN` `auth_middleware`。[1]
...
**`__Host-` / `__Secure-` 前缀**（`b26d81d`）：[1]
- `__Host-session_at` —— 强制 Secure + Path=/ + 无 Domain
- `__Secure-session_at` —— 强制 Secure（可有 Domain）
- 浏览器据前缀拒绝跨子域注入，弥补反向代理 + multi-tenant 场景的 cookie 注入风险[1]。

## WebSocket 单次性 ticket[1]

浏览器原生 WebSocket upgrade 不能带 `Authorization` 头。`ws_tickets.py` 提供独立的"鉴权后短期凭证"路径：[1]
...
- 4 个 WS 端点全部走 `_ws_auth_ok(request)` 校验[1]。

## Nous Portal Provider — 总是开启 + per-instance client_id[1]

`plugins/dashboard_auth/nous/__init__.py`（582 行）实现 `nous-account-service/docs/agent-dashboard-oauth-contract.md`（PR #180）。[1]
...
- 验证失败时 token 的 `iss` / `aud` 透传到错误消息便于诊断（`a498485`）[1]。

## Plugin Hook 集成[1]

`hermes_cli/plugins.py:558-594`：[1]
...
错误处理统一：异常吃掉变 warning，破窗 plugin 不让 host 崩溃。[1]

## fail-closed 与 hardening[1]
...
- **stub token 固定长度 sig 后缀**（`866cc98`）—— 防止 timing oracle 区分长度[1]。

## Phase 演进时间线（commits 同日落点）[1]
...
## 与 loopback `_SESSION_TOKEN` 的关系[1]
...
老 `_SESSION_TOKEN` 路径并未被删除 — 仅在 `auth_required=False` 时启用；gated mode 下被显式抑制不注入 SPA bundle。[1]

## 测试矩阵[1]

`tests/plugins/dashboard_auth/` 与 `tests/test_dashboard_*` 覆盖：[1]
...
- `strip HERMES_DASHBOARD_OAUTH_* env vars in hermetic fixture`（`c598076`）—— 防止 host 环境污染测试[1]。

## 2026-06-01 增量 — Dashboard 全管理面板复用既有闸门（#36704，`b571ec298`）[1]

`hermes_cli/web_server.py +786`（6300→7079 行）新增 17 个管理端点，全部经现有 middleware 走 OAuth 闸门 —— **无新增暴露面**：[1]
...
前端 4 个新页面 `web/src/pages/{McpPage,PairingPage,WebhooksPage,SystemPage}.tsx`（446 + 276 + 447 + 663 行）+ `web/src/App.tsx:78-81 import` + `:131-134` 路由 + `web/src/lib/api.ts +257` fetch wrappers。228 行测试 `tests/hermes_cli/test_dashboard_admin_endpoints.py`。[1]

### Dashboard OAuth `/api/*` 不在 next= round trip（`e1eba6f8c`，#36244）[1]
...
详见 [[2026-06-01-update#2-dashboard-全管理面板]]。[1]

---

## 2026-06-02 增量 — Refresh-token cookie 轮转（#37247，`c10ccaaf`）[1]

`feat(dashboard-auth): rotate dashboard sessions via refresh token (#37247)` —— 4 文件 +428 / -55。修复**浏览器在 `Max-Age` 后驱逐 access-token cookie**（refresh token cookie 还在）的硬 401 误判：之前 middleware 把这视为"未认证"强制完整重登；现在能用 RT 静默续期 AT。[1]

### 缘起（commit body 实证）[1]

老路径里 `_attempt_refresh` **只在 AT 存在但失效**时触发（即调 token-introspection 失败时）。但浏览器场景下 AT 在 Max-Age 后**直接消失**，根本没有 token 走到 introspection —— gate 看到无 cookie 就强制清掉 RT、跳 OAuth 重登。**手动 Refresh button 路径**和老回归测试都恰好让 AT 保持存在，所以这个失败模式逃过审查。[1]

### Provider 端（`plugins/dashboard_auth/nous/__init__.py`）[1]
...
### Middleware 端（`hermes_cli/dashboard_auth/middleware.py`）[1]
...
### 与 OAuth 一致的 `_token_response_to_session` 处理器（`:304-326`）[1]
...
—— 让 `complete_login()`（auth_code → InvalidCodeError）和 `refresh_session()`（RT → RefreshExpiredError）走同一段，**省 50+ 行重复**。[1]

### Cookie 体系（`hermes_cli/dashboard_auth/cookies.py`）[1]
...
- `:148-151` if `refresh_token`: → `cookies.set("refresh_token", refresh_token, ...)` —— 空串走 else 分支不 set[1]。
...
- `tests/plugins/dashboard_auth/test_nous_provider.py` **+113** —— RT 路径单元测试 + token-response 形态测试[1]。

### 影响范围[1]

- **不动 OAuth 流**：初始 auth_code → token 交换路径完全没改，新 hook 只在 RT 已经存在时触发
- **不动 cookie schema**：RT cookie 已经存在了一段时间，本次只是 middleware 终于学会**用**它
- **不动 Provider ABC**：`refresh_session` 是 `base.py:119` 已声明的 abstract 方法（详见 §4），Nous Provider 终于补上 concrete 实现[1]。

详见 [[2026-06-02-update#4-dashboard-auth-refresh-token-轮转]]。[1]
