...
Hermes 的多 Agent 能力分为 **5 种运行时机制**：4 种 in-process（单 session 内）+ 1 种跨 session 持久化（Kanban） [1]：
...
v0.13+ 引入 [Kanban Multi Agent Board](concepts/kanban-multi-agent-board.md) 作为**第 5 种**机制：把"AI 团队真正能跑完的项目板"做成一等公民 [1]。Kanban 不在本页详述，专门有页 [1]。
...
### LLM 自主调用（Delegate Task / MoA） [1]

和 `web_search`、`read_file` 完全一样 — 模型在系统 prompt 中看到工具描述，根据用户问题**自己判断**是否调用，没有任何代码逻辑强制触发 [1]。
...
`delegate_task` 在 `_invoke_tool()` 中有显式分支（line 6108），因为需要注入 `parent_agent` [1]。`mixture_of_agents` 走通用 registry dispatch [1]。

### 系统自动触发（Background Review） [1]

**LLM 不参与决定** [1]。两个独立计数器在主循环中静默递增，阈值到达后在**用户拿到回复之后**自动触发：
...
**关键区别**：用户已经拿到回复了，review 是后台静默行为 [1]。类似 GC（垃圾回收）— 定期自动跑，用户无感知 [1]。

## 一、Delegate Task — 子代理委派 [1]

Agent 运行时的核心多 Agent 能力 [1]。父 Agent 生成隔离的子 Agent 执行独立任务 [1]。
...
### Orchestrator 角色 + 可配置深度（v2026.4.18+） [1]

`delegate_task` 新增 `role` 参数，支持 `leaf`（默认）和 `orchestrator` [1]：
...
**默认扁平姿态**：`max_spawn_depth=1` 时，orchestrator 角色静默降级为 leaf [1]。用户主动把 `max_spawn_depth` 调到 2 或 3 才解锁嵌套委派 [1]。

新增 `DelegateEvent` enum（带 legacy 字符串向后兼容），供 gateway/ACP/CLI 进度消费者使用 [1]。

### 跨 Agent 文件状态协调（v2026.4.18+） [1]

多个并发子 agent 同时修改文件时，父 agent 现在能看到一致的文件状态 [1]：
- 子 agent 写入的文件对其他子 agent 可见 [1]
- 父 agent 收到汇总时能看到所有子 agent 的文件操作 [1]
- 防止并发 patch 丢失 [1]
...
两种模式 [1]：
- **单任务**：传 `goal`，直接执行（无线程池开销） [1]
- **批量**：传 `tasks` 数组，`ThreadPoolExecutor(max_workers=3)` 并行 [1]
...
子代理在 `_build_child_agent` 时注册到 `_active_children`，执行完成后在 `finally` 中注销 [1]。

### 结果结构 [1]

父 Agent 只看到这个结构化摘要，**不看到子代理的中间工具调用和推理** [1]：
...
通过 ACP 协议委派给外部 Agent（如 Claude Code） [1]：
...
Hermes 作为编排器，外部 Agent 作为执行者 [1]。

---

## 二、Mixture of Agents — 多模型协同推理 [1]

不是子代理，而是**多个外部 LLM 协作回答同一个问题** [1]。
...
## 三、Background Review — 后台经验提炼 [1]

Agent 在对话过程中**自动 fork 一个静默 Agent**，回顾对话并创建/改进 skill [1]。
...
Hermes 的 agent 间通信**没有消息队列、没有共享内存、没有 IPC**——全部在单进程内通过 Python 原生机制完成 [1]。

### Delegate Task 的通信 [1]

父子 agent 通过 **ThreadPoolExecutor + Future** 通信，本质是线程间函数调用 [1]：
...
**单任务时更简单**，连线程池都不用（line 612） [1]：
```python
result = _run_single_child(0, _t["goal"], child, parent_agent)
```

### Mixture of Agents 的通信 [1]

MoA 连线程都没有，是**同一线程内的异步 HTTP 请求 + 内存聚合** [1]：
...
4 个模型的中间回答存在函数栈的 list 里，聚合完成后垃圾回收，不落盘 [1]。

### 子 agent 之间 [1]

**完全不通信。** 多个子 agent 并行跑在各自的线程里，互不知道对方存在，没有任何协调机制 [1]。

### 中断时的结果处理 [1]

用户在子 agent 执行期间发新消息时 [1]：
...
中断后 [1]：
- 子 agent 返回 `status: "interrupted"`，已产出的部分结果保留在返回值中 [1]
- 父 agent 的 `_persist_session` 触发，把包含中断结果的消息写入 SQLite [1]
- **但中断结果不会作为有效答案展示给用户**——父 agent 标记 `completed: False`，进入处理新消息的下一轮 [1]
- 子 agent 自身 `persist_session=False`，不会独立写 DB——其部分结果以"父 agent 对话中的工具返回消息"形式存在 DB 里 [1]
...
**一句话：会话内的三种机制全部在单进程内完成，没有任何进程间通信。** [1] 唯一的例外是下面第七节的 Kanban 编排层——它是一个独立的**跨进程**多 Agent 机制 [1]。

---

## 七、Kanban 编排层 — 跨进程多 Agent [1]

前面六节描述的都是**会话内**机制 [1]。Kanban 编排器是一个**独立的、跨进程的**多 Agent 层：gateway dispatcher 在每个 tick 把任务从看板派发出去，并**spawn 独立的 worker 子进程**（`sys.executable -m hermes_cli.main`）来执行任务——这是真正的进程间编排，不是线程或协程 [1]。
...
### 自动分解（auto-decomposition） [1]

新建的 triage 任务会被自动分解成可执行的 workgraph [1]：

- **`kanban.auto_decompose`**（默认 `True`）— 是否启用自动分解 [1]
- **`kanban.auto_decompose_per_tick`**（默认 `3`）— 每个 dispatcher tick 最多分解的任务数 [1]
- `decompose_triage_task`（`kanban_db.py:2780`）把 triage 任务扇出成子任务，根据子任务描述把它们路由到对应的 specialist profile，根任务保留为父节点；所有子任务 `done` 后根任务升级为 `ready` [1]。

### Step-0 Profile 发现 [1]

编排器不使用 hardcoded 的 profile 名单，而是采用 **Step-0 profile discovery**：`_build_roster`（`kanban_decompose.py:214`）读取配置好的 profile roster，动态决定可用的 specialist profile 集合 [1]。

### 诊断与并发控制 [1]

- **`stranded_in_ready` 诊断**（`kanban_diagnostics.py:571`）：检测长期卡在 `ready` 状态没有 worker 接走的任务，阈值 `stranded_threshold_seconds`（默认 `1800` 秒 = 30 分钟） [1]。
- **`max_spawn`** 是一个**实时并发上限**（live concurrency cap），不是每 tick 的 spawn 预算——它限制的是同时运行的 worker 数量 [1]。

---

## 五、Goal Loop —— Ralph 循环 [1]

`/goal <text>` 进入 *持续目标* 模式：每个 turn 结束后辅助模型判官评判是否完成，未完成则注入续转 prompt（普通 user message），直到 done / budget / pause / cleared / 新用户消息抢占 [1]。

要点（**不**同于 Delegate / MoA / Background Review） [1]：

- **不 fork**：全程在主 session 里跑，prompt cache 持续命中 [1]
- **不改 system prompt / toolset**：续转 prompt 是 user message 追加 [1]
- **跨 turn 持久**：`state_meta` 表 `goal:<session_id>`，`/resume` 接续 [1]
- **fail-OPEN**：judge 出错 = continue，turn budget (默认 20) 是最后保险 [1]
...
v0.13.0 引入的 **durable multi-profile board**（`hermes_cli/kanban*.py` + `tools/kanban_tools.py`，~10.9k 行） [1]。

唯一**跨 session、跨 profile**的协作机制：

- SQLite 存储 (`<root>/kanban.db`) [1]
- dispatcher 守护进程从 `ready` 队列 spawn worker 子进程（独立 hermes 进程） [1]
- worker 通过 9 个 `kanban_*` tool 把状态写回 DB [1]
- 心跳 + reclaim + zombie detection + per-task `max_retries` + hallucination gate [1]
- dashboard / `/kanban` 斜杠 / `hermes kanban` CLI 三个面绕过 agent 直接操作 [1]
...
```text
父 Agent:  IterationBudget(90)   ← 默认 90 [1]
子代理 A:  IterationBudget(50)   ← 独立，不消耗父预算 [1]
子代理 B:  IterationBudget(50)   ← 独立 [1]
子代理 C:  IterationBudget(50)   ← 独立 [1]
──────────────────────────────
理论总迭代: 90 + 150 = 240 [1]
```
...
详见 → [Configuration And Profiles](concepts/configuration-and-profiles.md)
...
- [Configuration And Profiles](concepts/configuration-and-profiles.md) — 多 Profile 架构（另一种多 Agent 方案）
- [Tool Registry Architecture](concepts/tool-registry-architecture.md) — 子代理通过 registry 获取受限工具集
- [Auxiliary Client Architecture](concepts/auxiliary-client-architecture.md) — 子代理可配置独立的辅助模型
- [Credential Pool And Isolation](concepts/credential-pool-and-isolation.md) — 凭证池共享与轮换
- [Skills System Architecture](concepts/skills-system-architecture.md) — Background Review、kanban-orchestrator/worker skills
- [Trajectory And Data Generation](concepts/trajectory-and-data-generation.md) — Batch Runner（Nous 内部训练工具，不属于 Agent 运行时）
- [2026 05 07 Update](changelogs/2026-05-07-update.md) — Kanban 首次发布（v0.13.0）
- [2026 05 11 Update](changelogs/2026-05-11-update.md) — Kanban 安全 + worker mode 修复
