...
- [Worktree Isolation](concepts/worktree-isolation.md) — `.worktrees/` 兄弟目录被 checkpoint 主动排除
- [Security Defense System](concepts/security-defense-system.md) — `terminal` 的危险命令模式 + checkpoint 是回滚保险
- [Cli Architecture](concepts/cli-architecture.md) — `/checkpoints` 斜杠命令
