...
Hermes 的记忆系统是一个**三层架构**：存储层（MemoryStore）、编排层（MemoryManager）、插件层（MemoryProvider）[1]。
...
- **`MEMORY.md`**（默认上限 2200 字符）— Agent 的个人笔记（环境事实、项目约定、工具特性）[1]
- **`USER.md`**（默认上限 1375 字符）— 用户画像（偏好、沟通风格、期望）[1]
- 存储路径：`{HERMES_HOME}/memories/`[1]
- 条目分隔符：`§`（section sign），支持多行条目[1]
...
**为什么？** 保持系统提示稳定 → Anthropic prefix cache 不失效。写入立即持久化到磁盘，但当前会话的系统提示看不到自己的写入[1]。
...
读者总是看到完整的旧文件或完整的新文件，无中间状态[1]。

### 外部漂移防护（2026-05-23, PR #30877 / #26045）

原子写不能解决另一个问题：**MEMORY.md / USER.md 被 tool 之外的写者改了**（patch tool、用户手编、`cat >> MEMORY.md` 的 onboarding）[1]。
...
USER.md 同样保护。Tests 137 → 144（+7 个用例 pin 三种 mutator 的拒绝行为、`.bak.<ts>` 命名、false-positive 净化场景）[1]。

### 安全扫描（2026-05-26 起：双层 + Load-Time Snapshot 净化）

写入扫描与 snapshot 净化合走 `tools/threat_patterns.py`（v0.14.0 安全 wave + 2026-05-26 promptware defense 提取出的**共享威胁模式库**）[1]。

**Layer A — 写入扫描（旧路径，scope="strict"）**：每次 `memory(action=add/replace)` 调用 `first_threat_message(content, scope="strict")`，命中即 raise，content 永远到不了磁盘。Strict scope 包括经典 prompt injection / role-hijack / exfil curl-wget / read_secrets / authorized_keys / `update AGENTS.md` / hardcoded api_key 等[1]。

**Layer B — Load-Time Snapshot 净化（2026-05-26 NEW，commit `0dee92df2`）**：`MemoryStore.load_from_disk()`（`tools/memory_tool.py:133-172`）读 `MEMORY.md` / `USER.md` 后调 `_sanitize_entries_for_snapshot()`（line 174-208），**每 entry 跑 `scan_for_threats(entry, scope="strict")`**。命中即在 frozen system-prompt snapshot 中替换为[1]：
...
- `memory_entries` / `user_entries`（live state）—— 保留**原始**毒化文本。用户仍可 `memory(action=read)` 查看 + `memory(action=remove)` 删除[1]。
...
完整 250+ 模式与 invisible-unicode 集见 `tools/threat_patterns.py:49-137`[1]。
...
## 二、编排层：MemoryManager[1]

文件路径：`agent/memory_manager.py`（367 行）[1]
...
**"内置 + 至多一个外部"规则**：`add_provider()` 会拒绝第二个非 builtin provider，并打印警告[1]。
...
所有钩子**广播给全部 provider**，每个 provider 的失败被隔离（try/except，不传播）[1]：
...
## 三、插件层：MemoryProvider ABC[1]

文件路径：`agent/memory_provider.py`（232 行）[1]
...
### `on_session_switch` — Session ID 中途切换通知（v2026.4.23+）[1]

之前 provider 只在初始化时收到一次 `session_id`。但 `session_id` 会在 `/resume`、`/branch`、`/reset`、`/new`、上下文压缩等场景被**重新分配**——provider 不知道，后续写入会落到错误的 session 记录里[1]。

`agent/memory_manager.py:on_session_switch()` 现在会在 session_id 改变时调用所有 provider 的 `on_session_switch(new_session_id, parent_session_id, reset, **kwargs)`，让 provider 刷新缓存的 per-session 状态。Provider 不需要 tear down 重建，只需要更新内部句柄。错误会被 swallow（log debug 不阻塞主流程）[1]。

### initialize() 的 kwargs[1]

**始终提供**：`hermes_home`（HERMES_HOME 路径）、`platform`（"cli"/"telegram"/"discord"...）[1]

**可能提供**：`agent_context`（"primary"/"subagent"/"cron"/"flush"）、`agent_identity`（profile 名）、`agent_workspace`（共享工作区名）、`parent_session_id`（子代理的父 session）、`user_id`（平台用户 ID）[1]
...
插件发现机制：扫描 `plugins/memory/` 目录，找到含 `__init__.py` 的子目录，调用 `is_available()` 快速检查[1]。

### Hindsight Provider（v2026.5+ 增强）[1]

`plugins/memory/hindsight/__init__.py`：长期记忆 with 知识图谱、实体消歧、多策略检索。**Cloud + Local 两种模式**[1]。
...
Config fallback 链：`$HERMES_HOME/hindsight/config.json` (profile-scoped) → `~/.hindsight/config.json` (legacy shared)[1]。

**`update_mode='append'` 探测**（`feat(hindsight): probe API for update_mode='append' support, dedupe across processes` / 3082fa0）[1]：

启动时 probe `<api_url>/version`，gate 在 Hindsight ≥ 0.5.0[1]：
- **支持时**：稳定 session-scoped `document_id = session_id` + `update_mode='append'`，跨进程对同一 session 的 retain 合并到一条文档（不再产生 N 条 process-stamped 重复）[1]
- **不支持时**：`f"{session_id}-{start_ts}"` 唯一 document_id（resume-overwrite 修复 #6654 仍生效）[1]

`_append_capability_cache: Dict[str, bool]` 按 API URL 缓存能力，进程内 probe 一次[1]。

Plugin manifest hook：`on_session_end`[1]。
...
Memory 工具**不在 tool registry 中**。它在 `run_agent.py` 中被显式拦截[1]：
...
**为什么不走 registry？** 因为 memory 工具需要直接访问 `self._memory_store` 实例，而 registry 的 handler 签名不传 agent 内部状态[1]。
...
系统每 10 轮（`_memory_nudge_interval`）自动触发一次后台 review[1]：
...
Review Agent 以 `_MEMORY_REVIEW_PROMPT` 审视对话历史，自动调用 memory 工具提取持久事实[1]。

---

## 五、LLM 看到的 Memory 工具[1]
...
MemoryManager 中每个 provider 方法调用都在 try/except 中。一个 provider 崩溃不影响其他 provider，不阻塞 Agent 执行[1]。
...
**仅 add 和 replace 触发镜像，remove 不触发。**[1]

### 预取缓存[1]

`prefetch_all()` 在每次 API 调用前调用一次，结果缓存到 `_ext_prefetch_cache`。同一轮内多次 tool call 不会重复预取（避免 10 次工具调用 = 10 倍延迟）[1]。
...
**通过工具返回值兜底。** 每次 `memory(action="add/replace/remove")` 调用后，返回值包含**实时的全部条目**[1]：
...
模型在对话上下文中已经能看到最新内容，不需要系统提示更新[1]。
...
**硬拒绝 + 返回当前条目让模型自己管理空间。** 没有自动淘汰、没有 LRU、没有溢出[1]。
...
**设计意图**：Memory 不是数据库，是**精心策展的小卡片盒**。限制空间逼模型做策展 — 过时的 replace、不重要的 remove、新发现的 add[1]。

### Q3：更多历史信息怎么办？

Memory 只存**持久事实**（2200 + 1375 字符）。更大量的历史信息通过 **session_search 工具**检索[1]。

两者分工明确，系统提示里显式指导模型[1]：
...
## 九、与 Session Search 的关系[1]

Session Search 不是 Memory 的一部分，但是 Memory 系统的**互补机制**[1]。
...
**注意**：Session Search 是 FTS5 关键词匹配，不是语义向量搜索。搜 "nginx 配置" 不会匹配只写了 "反向代理" 的会话。搜索语法支持 `OR`、`NOT`、`"精确短语"`、`前缀*`[1]。

两种模式：
- **空 query** → 列出最近会话（零 LLM 成本，只返回标题/预览/时间戳）[1]
- **有 query** → FTS5 搜索 + 并行 LLM 摘要（最多 3-5 个会话）[1]
