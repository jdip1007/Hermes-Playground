...
- **Telegram /topic（DM topic mode）**（d6615d8、d35efb9）：在私聊里通过 `/topic <name>` 创建 Hermes 管理的伪话题（forum topic），每个话题独立 session。`/topic off` 退出，`auth gate + screenshot debounce`，CASCADE 删除，rename guard，General-topic 处理。详见 [Gateway Session Management](gateway-session-management.md)。
...
- **Webhook 路由 fail closed + Svix 签名**（`bbf02c3`/`dbf73e9`/`15aa688`）：见 `[Security Defense System](security-defense-system.md)` 同名段落 —— `gateway/platforms/webhook.py:383-395 + 690+` 增 Svix 签名 + 缺 secret 返 403。
...
- **Plugin transform_llm_output 流式可见性**：见 `[Interrupt And Fault Tolerance](interrupt-and-fault-tolerance.md)` "Streaming 完成可见性" 章节 —— `gateway/run.py:17680-17717` `_transformed=True` 时编辑已 streamed message in-place 而非发重复，避免 plugin 修改不可见。
...
四层防御链（详见 [Security Defense System](security-defense-system.md)）：
...
- [Gateway Session Management](gateway-session-management.md) — 网关会话管理架构
- [Cron Scheduling](cron-scheduling.md) — Cron 调度器由网关驱动
- [Hook System Architecture](hook-system-architecture.md) — 网关事件钩子系统
- [Security Defense System](security-defense-system.md) — Webhook 硬化、Gateway 平台审批授权链、WeCom callback defusedxml、API server key placeholder
- [Dashboard Auth Oauth Gate](dashboard-auth-oauth-gate.md) — Dashboard 鉴权独立闸门（与 api_server Bearer scheme 互补）
