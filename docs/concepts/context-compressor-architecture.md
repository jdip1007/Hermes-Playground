...
- [Auxiliary Client Architecture](auxiliary-client-architecture.md) — 压缩通过 `call_llm(task="compression")` 调用
- [Smart Model Routing](smart-model-routing.md) — 使用 get_model_context_length() 获取上下文窗口
- [Prompt Builder Architecture](prompt-builder-architecture.md) — 压缩后的消息传给 prompt builder 重建提示
- [Prompt Caching Optimization](prompt-caching-optimization.md) — 压缩策略与 prompt caching 协调
- [Large Tool Result Handling](large-tool-result-handling.md) — 工具输出修剪与大型结果处理理念相通
- [Session Search And Sessiondb](session-search-and-sessiondb.md) — Session 分裂后原始消息保留在 DB 中供检索
- [Memory System Architecture](memory-system-architecture.md) — 压缩前 `on_pre_compress` 通知（`flush_memories` 工具已在 v2026.4.30 移除）
