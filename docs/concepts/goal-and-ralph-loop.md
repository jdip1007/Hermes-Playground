...
`/goal` 把"持续工作直到目标完成"做成一等公民 [1]。用户设定一个标准目标后，每回合结束都由辅助 LLM **判官** 评判是否达成：

- **达成** → 停下来告诉用户 [1]
- **未达成** → 自动把"继续推进目标"作为下一回合 user message 注入 [1]

这是 **Ralph loop** 模式的官方实现，在 Hermes v0.13 (`#18262/#18275`) 引入，v0.14 (`#25449`) 补全 `/subgoal` [1]。

> 关键不变量：续推 prompt 是普通 user message，不动 system prompt → **保护 prompt cache** [1]。

源码：`hermes_cli/goals.py`（762 行）+ CLI/Gateway/TUI 三处钩子 [1]。
...
Subgoals 是**约束**：每条都要在 agent 的响应中有具体证据，才算完成 [1]。判官明确要求"不接受 'all requirements met' 这种空洞话" [1]。
...
序列化：`to_json()` / `from_json()` 在 `goals.py:163-185`，带向后兼容反序列化 [1]。

---

## 持久化

存在 SessionDB `state_meta` 表，key `goal:<session_id>` [1]：
...
**保存时机**：每次判官调用后（`goals.py:727`）+ 任何状态变更（设置/暂停/完成/清除/加 subgoal/删 subgoal）[1]。

SessionDB 连接按 `hermes_home` path 缓存（`goals.py:206-236`），保证 profile 切换正确 [1]。
...
1. **空校验**（`goals.py:398-402`）：goal 或 response 为空 → 返回 `"continue"`，跳过判官调用 [1]
2. **客户端解析**（`goals.py:410-417`）：`auxiliary_client.get_text_auxiliary_client("goal_judge")` 路由判官模型 [1]
   - 无可用客户端 → 返回 `"continue"`（**fail-open**）[1]
3. **Prompt 选择**（`goals.py:419-437`）[1]
   - 有 subgoals → `JUDGE_USER_PROMPT_WITH_SUBGOALS_TEMPLATE`（goals.py:120-134）
   - 否则 → `JUDGE_USER_PROMPT_TEMPLATE`（goals.py:111-116）
4. **API 调用**（`goals.py:439-450`）：temperature=0，system prompt `JUDGE_SYSTEM_PROMPT`（goals.py:95-108），max_tokens 默认 4096，timeout 30s [1]
5. **解析**（`_parse_judge_response`，`goals.py:322-368`）：单行 JSON `{"done": <bool>, "reason": "<text>"}` [1]
   - 容忍 markdown fence
   - 容忍前置散文
   - `"true"/"yes"/"1"/"done"` 字符串自动转 bool
   - **解析失败 → `parse_failed=True`**，verdict=continue（fail-open）[1]
...
包含编号 subgoal 列表 + 必须全满足的措辞 [1]。

**这些都是普通 user message**，作为下一回合输入注入 [1]。不动 system prompt = 不毁 prompt cache [1]。

---

## 自动暂停

### 预算耗尽（`goals.py:711-725`）[1]

`turns_used >= max_turns` 时：
- status → `paused` [1]
- paused_reason → `"turn budget exhausted (n/20)"` [1]
- 提示用户 `/goal resume` 或 `/goal clear` [1]

### 判官连续解析失败（`goals.py:687-709`）[1]

`consecutive_parse_failures` 达 `DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES`（默认 3）时：
- status → `paused` [1]
- paused_reason → `"judge model returned unparseable output N turns in a row"` [1]
- 提示用户给 `auxiliary.goal_judge` 配更严谨的模型 [1]

### 用户 Ctrl+C 中断（`cli.py:8944-8953`）[1]

回合被 Ctrl+C 打断 → 不判，直接暂停 [1]。

---

## 三处钩子

`/goal` + `/subgoal` 在 CLI、Gateway、TUI 都注册，语义一致 [1]：
...
Gateway 用 `MessageEvent` FIFO 队列实现相同抢占语义 [1]。
...
`max_tokens` 默认 4096 而不是 200：DeepSeek-V4 / QwQ 等推理模型会先 emit 大量隐藏 token 再写 JSON，200 token 会被截断 [1]。
...
**没有直接依赖**。`conversation_loop.py` 不 import `goals` [1]。续推完全是 **post-turn**：

这种**调用者驱动**的设计：

1. 不需要改 agent loop [1]
2. 不污染 system prompt → prompt cache 完整 [1]
3. 用户实时输入自然抢占（real user message 排在续推前）[1]

---

## 设计原则

1. **Fail-open**：判官出错（API/parse） → 默认 continue，从不假阳性 done。Turn budget 是兜底 [1]
2. **不改 system prompt**：续推是 user message，保 cache [1]
3. **用户抢占**：真实 user message 自动挤掉续推 [1]
4. **CLI / Gateway / TUI 完全对齐**：同一套语义 [1]
5. **无状态判官**：每回合独立调用，不带历史，temperature=0 [1]
6. **Subgoals 是约束不是支线**：必须全有证据，否则继续 [1]
