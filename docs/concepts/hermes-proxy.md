...
`hermes proxy` 把 OAuth 订阅（Nous Portal、xAI SuperGrok，未来扩展 Claude Pro / ChatGPT Pro）**暴露成 OpenAI-compatible 本地 HTTP 端点** [1]。任何认 OpenAI API 的客户端（Codex CLI、Aider、Cline、Continue、自家脚本）都能直接接，无需 API key，复用你已有的订阅 [1]。

v0.14 落地（PR `#25969`）。代码在 `hermes_cli/proxy/` [1]。
...
剥离客户端 Authorization header → 注入上游 OAuth bearer → 透传 SSE/JSON 原样。无任何 prompt 改写、计费拦截或 logging [1]。
...
`status` 输出 adapter 就绪情况 + token 过期时间 [1]。
...
`NousPortalAdapter` 类在 `adapters/nous_portal.py:50`，`allowed_paths` property 在 line 67 返回 module-level `_ALLOWED_PATHS`（line 40-47） [1]。
