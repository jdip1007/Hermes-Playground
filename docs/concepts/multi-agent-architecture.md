...
v0.13+ 引入 [Kanban Multi Agent Board](kanban-multi-agent-board.md) 作为**第 5 种**机制：把"AI 团队真正能跑完的项目板"做成一等公民 [1]。Kanban 不在本页详述，专门有页 [1]。
...
详见 → [Configuration And Profiles](configuration-and-profiles.md)
...
- [Configuration And Profiles](configuration-and-profiles.md) — 多 Profile 架构（另一种多 Agent 方案）
- [Tool Registry Architecture](tool-registry-architecture.md) — 子代理通过 registry 获取受限工具集
- [Auxiliary Client Architecture](auxiliary-client-architecture.md) — 子代理可配置独立的辅助模型
- [Credential Pool And Isolation](credential-pool-and-isolation.md) — 凭证池共享与轮换
- [Skills System Architecture](skills-system-architecture.md) — Background Review、kanban-orchestrator/worker skills
- [Trajectory And Data Generation](trajectory-and-data-generation.md) — Batch Runner（Nous 内部训练工具，不属于 Agent 运行时）
